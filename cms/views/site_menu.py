...
class SiteMenu(AbstractMenu):
    ...
    def show(self):
        ...
        if AppContext().permission_repo.has_permission(
            ...
            if self.logged_user.username == self.selected_site.owner.username:
                options.extend(
                    [{"message": "Adicionar Gerente", "function": self._add_manager}])
    ...
    def _add_manager(self):
        ...
        for idx in selected_indexes:
            ...
            if 1 <= n <= len(users):
                user = users[n - 1]
                print(
                    f"Permissão de gerência dada ao usuário {user.username}.")
    ...
    def _select_post(self):
        posts: list[Post] = AppContext(
        ).post_repo.get_site_posts(self.selected_site)
        ...
        def execute_for_option(selected_post: Post):
            ...
            PostMenu(self.logged_user, self.selected_site,
                     selected_post).show()
